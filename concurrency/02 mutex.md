...
Далее планировщик ОС возвращается к выполнению инструкции № 2 нашего потока и сравнивает значения регистра rax с нулём, но, так как другой поток уже изменил значение count, то наше сравнение сделает ошибочный вывод. Мы выполним ветку для **count == 0**.
