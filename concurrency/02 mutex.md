...
Инструкция [lock; cmpxchg (lock_addr) %rcx](https://www.felixcloutier.com/x86/cmpxchg) атомарно сравнивает значение в регистре rax с первым операндом (ячейкой памяти), если значения равны, то второй операнд загружается в первый операнд. В противном случае первый операнд загружается в регистр rax. 
