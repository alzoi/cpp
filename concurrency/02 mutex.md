...
_start:
    ...
    # и если там ноль, то записываем в ячейку значение из rcx.  
    lock cmpxchg %rcx, lock_addr(%rip) 
    
    # Переход к метке L2, если ZF не равно нулю (if ZF != 0)
    jne .L2_no_locked            
...
Вывод:
  код 1 - блокировку захватить не удалось, критическая секция блокирована другим потоком.
```
