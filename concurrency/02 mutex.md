...
Важно учитывать, что операция проверки условия выполняется не атомарно
```cpp
if (count == 0) {
}
```
указанная операция в машинных кодах разбивается на следующие инструкции:
```asm
# 1) Извлекаем из памяти по адресу count значение в регистр процессора eax
move count(%rip), %rax
# 2) Сравниваем значение регистра eax с нулём.
cmp $0, %rax
```
После выполнения первого этапа извления значения из памяти,  
ОС может переключиться на выполнение другого потока.
