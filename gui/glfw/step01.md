...
int main(void) try {
  ...
  // Обычно монитор работает с частотой от 60 Гц, то есть 60 раз в секунду дисплей обновляет все пиксели монитора.
  // В следующей функции, мы указываем, что нужно ждать 30 тактов обновления развёртки монитора,
  // а затем перерисовать сцену функцией glfwSwapBuffers.
  glfwSwapInterval(30);
  ...
  while (!glfwWindowShouldClose(lo_window1) && !glfwWindowShouldClose(lo_window2)) {
      ...
      // Для отображения частоты перерисовки сцены.
