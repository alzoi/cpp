...
void render_openGL() {
  // Функция отрисовки сцены функциями OpenGL, сцена создаётся во внеэкранном буфере (невидимом, заднем буфере).
...
int main(void) {
    ...
    while (!glfwWindowShouldClose(lo_window)) {
        ...
        // Скопировать данные внеэкранного буфера на экранный буфер (передний буфер).
